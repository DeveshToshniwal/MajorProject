        ...
        for(let i=0;i< loanCount.valueOf();i++)
        {
          contractInstance.getLoanDetailsByAddressPosition.call(account, i).then(function(el) {
            ...
            var newRowContent = '<tr class="'+LOANSTATECLASS[el[0].valueOf()]+'">\
              ...
              <td>'+el[2].valueOf()/wtoE+' eth</td>\
              <td>'+el[7].valueOf()+'</td>\
              <td><a target="_blank" href="http://localhost:8080/verify.html?hash='+web3.toUtf8(el[5].valueOf())+web3.toUtf8(el[6].valueOf())+'">View</a></td>\
...
function newLoan(amount,cs, date, m1,m2) {
  CrowdBank.deployed().then(function(contractInstance) {
    // contractInstance.defaultAccount = account;
    contractInstance.newLoan(web3.toWei(amount,'ether'),cs,date,m1,m2,{gas: GAS_AMOUNT, from: account}).then(function() {
      refreshPage();
    });
  });
}
...
$( document ).ready(function() {
  ...
  document.getElementById('cs').value=params.get('cs');
  ...
  document.getElementById('newloan-form').addEventListener('submit', function(evt){
    ...
    var cs=$('#cs').val();
    var m1=mortgage.substr(0,32);
    var m2=mortgage.substr(32);
    newLoan(amount,cs,date,m1,m2);
  });
