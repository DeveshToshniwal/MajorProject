...
contract CrowdBank {
    ...
    constructor() public{
     ...
     function newLoan(uint amount, uint creditscore,uint dueDate, bytes32 m1, bytes14 m2) public {
        if(hasActiveLoan(msg.sender)) return;
        uint currentDate = block.timestamp;
        loanList.push(Loan(msg.sender, LoanState.ACCEPTING, dueDate, amount, 0, 0, currentDate, creditscore, m1,m2));
     ...
     function acceptProposal(uint proposeId) public
    {
     ...
     function getProposalAtPosFor(address lender, uint pos) public view returns(address, uint, ProposalState, uint, uint, uint, uint, bytes32,bytes14,uint) {
        Proposal storage prop = proposalList[lendMap[lender][pos]];
        return (prop.lender, prop.loanId, prop.state, prop.rate, prop.amount, loanList[prop.loanId].amount, loanList[prop.loanId].dueDate, loanList[prop.loanId].m1,loanList[prop.loanId].m2, loanList[prop.loanId].creditscore);
    }
...
// BORROWER ACTIONS AVAILABLE    
     ...
     function getLoanDetailsByAddressPosition(address borrower, uint pos) public view returns(LoanState, uint, uint, uint, uint,bytes32,bytes14,uint) {
        uint x=loanMap[borrower][pos];
        Loan storage obj = loanList[x];
        return (obj.state, obj.dueDate, obj.amount, obj.collected, x, obj.m1,obj.m2,obj.creditscore);
    }
     ...
     function getLastLoanDetails(address borrower) public view returns(LoanState, uint, uint, uint, uint,uint) {
        uint loanLength = loanMap[borrower].length;
        Loan storage obj = loanList[loanMap[borrower][loanLength -1]];
        return (obj.state, obj.dueDate, obj.amount, obj.proposalCount, obj.collected, obj.creditscore);
    }
